...
interface ImportModelsFormProps {
  categories: Category[];
  onImport: (newModels: Partial<Model>[]) => Promise<void>;
  onBack: () => void;
}
...
const REQUIRED_HEADERS = ['name', 'brand', 'categoryName'];
const ALL_HEADERS = ['name', 'brand', 'modelNumber', 'categoryName', 'specifications', 'imageUrl'];

const ImportModelsForm: React.FC<ImportModelsFormProps> = ({ categories, onImport, onBack }) => {
  ...
  const categoryMapByName = useMemo(() => new Map(categories.map(c => [c.name.toLowerCase(), c])), [categories]);
  ...
  const processFile = async (fileToProcess: File) => {
    ...
    try {
        ...
        const results: ValidationResult[] = lines.map((line) => {
            ...
            const category = categoryMapByName.get(row.categoryName.toLowerCase());
            if (!category) {
                return { status: 'error', message: `Catégorie "${row.categoryName}" non trouvée.`, data: {} };
            }
  ...
  const handleDownloadTemplate = () => {
    ...
    link.setAttribute("download", "import_models_template.csv");
  ...
  const handleImport = async () => {
    ...
    if (validItems.length > 0) {
      ...
      addToast('Aucun modèle valide à importer.', 'info');
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100">
      <PageHeader title="Importer des modèles" onBack={onBack} />
      <main className="flex-grow p-4 overflow-y-auto pb-24">
          ...
          {isProcessing && <p className="text-center text-gray-600">Traitement du fichier...</p>}
              ...
              <p className="text-gray-600 mt-2">Nous avons validé votre fichier. {validCount} modèle(s) prêt(s) à être importé(s), {errorCount} erreur(s) trouvée(s).</p>
                ...
                <table className="min-w-full divide-y divide-gray-200 text-sm">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left font-medium text-gray-500">Nom du modèle</th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {validationResults.map((result, index) => (
                      <tr key={index} className={result.status === 'error' ? 'bg-red-50' : 'bg-white'}>
                        ...
                        <td className="px-4 py-2 text-gray-700">{result.data.brand || 'N/A'}</td>
      ...
      <footer className="sticky bottom-0 bg-white/80 backdrop-blur-sm p-4 border-t border-gray-200">
        <div className="max-w-4xl mx-auto flex justify-end">
          <Button onClick={handleImport} disabled={validCount === 0 || isProcessing || isSaving} loading={isSaving} icon="cloud_upload">
            {isSaving ? 'Importation...' : `Importer ${validCount > 0 ? `(${validCount})` : ''} modèle(s)`}
          </Button>
        </div>
      </footer>
    </div>
...
export default ImportModelsForm;